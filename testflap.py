...
class SmartBird (Bird, Brain):
    ...
    def get_dist (self, rects):
        ...
        i, j = 0, 1
        while i < len(rects) and j < sz0 - 1:
            #skipping rectagnels that the bird passed
            while self.x > rects[i][0] + rect_width:
                i += 1

            #self.dist[j] = 2 * rects[i][0] / sqrt(height**2 + width**2) 
            #self.dist[j+1] = 2 * (rects[i][0] + rect_width) / sqrt(height**2 + width**2) 

            self.dist[j] = ( self.norm2 (rects[i][0], rects[i][1]) 
                    / sqrt(height**2 + width**2) )
            self.dist[j+1] = ( self.norm2 (rects[i][0] + rect_width, rects[i][1]) 
                    / sqrt(height**2 + width**2) )
            #self.dist[j+2] = ( self.norm2 (rects[i][0], rects[i][1] + ygap) 
            #        / sqrt(height**2 + width**2) )
            #self.dist[j+3] = ( self.norm2 (rects[i][0] + rect_width, rects[i][1] + ygap) 
            #        / sqrt(height**2 + width**2) )
            i += 1
            j += 1
...
def game (birds, attempts):
    ...
    while running:
        ...
        if allDead (birds):
            ...
            if gen_count > attempts: 
                running = False
            gen_count += 1
            print('Generation {0}/{1}'.format(gen_count,attempts), end='\r')
...
def play (szh, attempts):
    ...
    game (birds,attempts)
    birds.sort (key = lambda bird: bird.top_score, reverse = True)
    print('Brain shape: {0} \t Top scores: \n\t First: {1} \n\t Second: {2} \n\t Third: {3}'
            .format(dims, birds[0].top_score, birds[1].top_score, birds[2].top_score))


if __name__ == '__main__':
    threads = []
    for szh in range(2,18):
        threads.append(threading.Thread(target = play, args = (szh, 500)))
        threads[-1].start()
